...
public class MyJsonOutputFormat extends FileOutputFormat<NullWritable, Text> {

	
	public static class MyJsonRecordWriter extends RecordWriter<NullWritable, Text> {

		// flux de sortie (le fichier dans lequel il va ecrire (dans HDFS)
		private DataOutputStream out;
		private boolean isFirst;
		
		public MyJsonRecordWriter(DataOutputStream out) throws IOException {
		...
		public void write(NullWritable cle, Text value) throws IOException,
			...
			for (int i = 0; i < champs.length; i++) {
				out.writeBytes("col"+ i + ":");
				out.writeBytes("\""+ champs[i] + "\"");
	...
	public RecordWriter<NullWritable, Text> getRecordWriter(
		...
		// HDFS
		return new MyJsonRecordWriter(sortie);
	}
